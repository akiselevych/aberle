...
import close from "../../../../../public/assets/icons/closed.svg"
//styles
...
const QuetionItem: FC<Props> = ({ text, answ }) => {
  ...
  return (
    <div onClick={() => setIsOpen(prev => !prev)} className={`${styles.quetionsItem} ${isOpen && styles.activeItem}`}>
      <div className={styles.topRow}>
          <Image alt={isOpen ? 'open' : 'close'} src={isOpen ? open : close} />
        <p className={`${global.h3} ${styles.qutionText}`}>{text}</p>
      </div>
      {isOpen &&
